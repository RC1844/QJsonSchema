...
int main(int argc, char *argv[]) {
  ...
  // 测试1：基本字符串验证
  {
    ...
    QJsonSchema schema(schemaObj);

    // 测试有效字符串
    QString error;
    bool result = schema.validate("hello", &error);
    qDebug() << "验证 'hello':" << (result ? "通过" : "失败")
             << (result ? "" : error);

    // 测试过短字符串
    result = schema.validate("hi", &error);
    qDebug() << "验证 'hi':" << (result ? "通过" : "失败")
             << (result ? "" : error);

    // 测试过长字符串
    result = schema.validate("thisiswaytoolong", &error);
    qDebug() << "验证 'thisiswaytoolong':" << (result ? "通过" : "失败")
             << (result ? "" : error);
  }
  ...
  // 测试2：数值验证
  {
    ...
    SwJsonSchema schema(schemaObj);

    QString error;

    // 测试有效数值
    bool result = schema.validate(25, &error);
    qDebug() << "验证 25:" << (result ? "通过" : "失败")
             << (result ? "" : error);

    // 测试无效数值（不是5的倍数）
    result = schema.validate(23, &error);
    qDebug() << "验证 23:" << (result ? "通过" : "失败")
             << (result ? "" : error);

    // 测试超出范围数值
    result = schema.validate(150, &error);
    qDebug() << "验证 150:" << (result ? "通过" : "失败")
             << (result ? "" : error);
  }
  ...
  // 测试3：对象验证
  {
    ...
    qDebug() << "验证缺少name的对象:" << (result ? "通过" : "失败")
             << (result ? "" : error);
  }
  ...
  // 测试5：枚举验证
  {
    ...
    qDebug() << "验证 'red':" << (result ? "通过" : "失败")
             << (result ? "" : error);

    // 测试无效枚举值
    result = schema.validate("yellow", &error);
    qDebug() << "验证 'yellow':" << (result ? "通过" : "失败")
             << (result ? "" : error);
  }
